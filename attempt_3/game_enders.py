                ...
                for col in cols:
                    ...
                    if x_piece_can_move[xth_piece] == False:
                        if not is_friendly_fire(col, row, board, stalemated):
                            a = np.copy(board)
                            if a[piece] != stalemated[-1]:
                                a[row, col] = board[piece]
                                a[piece] = 0
                            ...
                            # board_2[target] = board_2[piece]
                            # board_2[piece] = board[target]

                            # if not np.isin(True, in_check(board_2, stalemated, stalemater_pieces)):
                            #     x_piece_can_move[xth_piece] = movement_list[int(i)](piece, target, board, stalemated, stalemater_pieces)
                            
                            # if x_piece_can_move[xth_piece] == True:
                            #     print(xth_piece, movement_list[int(i)], piece, board[piece], target, board[target])
